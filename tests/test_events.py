...
class TestEventConstruction:
    ...
    def test_tool_result(self):
        tr = ToolResult(tool_call_id="tc_1", tool_name="read_file", output="contents", is_error=False)
        assert tr.type == EventType.TOOL_RESULT
        assert not tr.is_error

    def test_tool_result_error(self):
        tr = ToolResult(tool_call_id="tc_1", tool_name="bash", output="Error: timeout", is_error=True)
        assert tr.is_error
...
class TestEventSerialization:
    @pytest.mark.parametrize("event", [
        ...
        ProviderMeta(provider="openai", model="gpt-4o", usage={"input_tokens": 1}, request_id="r1"),
...
class TestStopReason:
    def test_all_values(self):
        expected = {"end_turn", "tool_use", "max_tokens", "timeout", "max_steps", "cancelled", "error"}
